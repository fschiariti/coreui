...
export class FacturasService {
    ...
    public GetAll() {
        ...
        return this.http.get<FacturasModel[]>(url,{ headers: headers }).pipe(tap(data => data),
    ...
    // Update
    public Update(model: FacturasModel) {
        var putUrl = this.apiUrl + '/' + model.id_comp;
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.usuario.token}`);
    ...
    // Delete
    public Delete(id_comp) {
        var deleteUrl = this.apiUrl + '/' + id_comp;
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.usuario.token}`);
